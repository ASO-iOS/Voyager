...
struct Stage4SlotReference: PhraseDelegate {
    ...
    private let textSide0 = PhraseData(
        text: "На выходе из казино ты заметил троих мужчин у здания заведения. Двое блокировали выход третьему, зажимали его спиной к стене и агрессивно донимали расспросами. Неожиданно в тебе проснулся моралист и ты решил поставить их в известность, что двое на одного – не честно, и вообще бить людей это плохо.",
        type: .common
    )
    
    private let textSide1 = PhraseData(
        text: "В ответ тебе коротко и ясно дали понять не лезть в чужие дела, так как ты не знаешь, что вообще происходит. Оставалось принять решение, последовать ли совету",
        type: .common,
        answerPositive: "Согласиться",
        answerNegative: "Послать незнакомца",
        answerNeutral: "Воздержаться",
        hasAnswers: true
    )
    
    private let textSideNegative0 = PhraseData(
        text: "Действовать нужно было, пока никто не умер. Ты с разбегу влетел в толпу и ударил светом фонарика по лицам тех двоих. Так как дело было вечером, а действовал ты быстро, это их дезориентировало на пару мгновений, за которые тот другой, на которого наехали, успел вырваться из окружения и скрыться за углом, но вот самому тебе уже было не убежать.",
        type: .common
    )
    private let textSideNegative1 = PhraseData(
        text: "Как выяснилось, ты помог проигравшемуся должнику, а те двое просто хотели вернуть своё. Теперь им придётся ловить его заново, а тебя, чтобы больше не совался, придётся слегка проучить...",
        type: .common
    )
    
    private let textSideNeutral = PhraseData(
        text: "Ты на самом деле ничего в происходящем не понимал и людей этих ты не знал. Так что ну его, пускай разбираются, а ни то ещё зацепят тебя ненароком.",
        type: .common
    )
    
    private let textSidePositive0 = PhraseData(
        text: "Что бы там не происходило, ключевым моментом было то, что ты не боксёр. А ведь если тебя побьют, так ещё и деньги могут отнять, а этого допускать нельзя уж совсем. Так что ты отошёл в сторонку и призвал на помощь людей в погонах.",
        type: .common
    )
    
    private let textSidePositive1 = PhraseData(
        text: "Те объявились быстро и взяли всех троих до того, как кто-то остался с разбитым носом и другими тяжкими телесными. Оказалось, что все трое были криминальными элементами, или по-просту ворами. Жертва наезда оставила следы преступления на последнем деле, так что это угрожало поимкой всем участникам. Но теперь это уже было не важно, так как ты помог их найти. Тебе выдали небольшую премию за бдительность и грамоту в качестве благодарности.",
        type: .common
    )
    
    var phrases: [PhraseData] {
        return [text0, text1, text2, text3, text4, text5, text6, text7, textSide0, textSide1, textSideNegative0, textSideNegative1, textSideNeutral, textSidePositive0, textSidePositive1
        ]
    }
}
