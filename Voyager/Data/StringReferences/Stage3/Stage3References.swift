...
struct Stage3HippodromeReference: PhraseDelegate {
    ...
    private let textSide0 = PhraseData(
        text: "После просмотра скачек ты решил освежиться и зашёл в прилегающий бар неподалёку.",
        type: .common
    )
    
    private let textSide1 = PhraseData(
        text: "Почти сразу подошёл незнакомец с предложением поставить на победителя следующего забега. Вся ситуация могла указывать на один из двух вариантов: либо перед тобой обманщик, так как сейчас контроль плотный и подстроенных побед нет, либо этот парень является частью цепочки, которая успела выстроиться поверх нового порядка проверки участников, и тогда он может помочь поставить на своего рода слитую гонку. ",
        type: .common
    )
    private let textSide2 = PhraseData(
        text: "Оставался и третий вариант: никаких закулисных движений нет, он просто фанат гонок, который знал всех лошадей, и сейчас, пока игры честные, он хотел скорее заработать на любимом фаворите. Как бы там ни было, он предлагал ставить на его кандидата и просил денег.",
        type: .common
    )
    
    private let textSide3 = PhraseData(
        text: "Что же делать в такой ситуации?",
        type: .common,
    ...
    private let textSideNegative = PhraseData(
        text: "Недолго думая, ты решил, что всё это от лукавого, и ставить нужно своими силами, используя интуицию и данные самостоятельно. Ты отправил незнакомца с его предложением лесом. Это оказалось верным решением, ведь незнакомец проиграл. Вот только это его раздосадовало до такой степени, что, на выходе из бара, он решил присвоить себе твои деньги, угрожая украденным ножом. Тут уже деваться было некуда, пришлось поделиться.",
        type: .common
    )
    
    private let textSideNeutral = PhraseData(
        text: "Всё это выглядело не проверено и странно, вникать или доверяться не хотелось, так что ты решил выкрутиться самым простым способом – просто сказал, что всё проиграл и денег уже нет, а последние копейки ты потратил на пиво, чтобы залить горечь сегодняшних неудач. Незнакомец поверил, видать, такое тут часто случается.",
        type: .common
    )
    
    private let textSidePositive = PhraseData(
        text: "Так как теперь с деньгами было посвободнее, ты решил довериться тому, кто следил за гонками дольше тебя. Результат оправдался мгновенно – его кандидат стал чемпионом! Тебе удалось и помочь человеку, и заработать свою долю  участия.",
        type: .common
    )
    
    var phrases: [PhraseData] {
        return [text0, text1, text2, text3, text4, text5, textSide0, textSide1, textSide2, textSide3, textSideNegative, textSideNeutral, textSidePositive]
    }
}
